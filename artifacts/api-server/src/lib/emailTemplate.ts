/**
 * Ghostspere email template — generates HTML + plain-text emails for ticket dispatch.
 */
...
function tierBadge(tier: string): string {
  ...
  const bg = TIER_BG[tier] ?? "#0d0d0d";
  return `<span style="display:inline-block;padding:2px 10px;border-radius:4px;border:1px solid ${color};color:${color};background:${bg};font-size:11px;font-family:monospace;font-weight:700;letter-spacing:1px;text-transform:uppercase">${tier}</span>`;
}
        ...
        <table width="100%" cellpadding="0" cellspacing="0">
          <tr>
            ...
            <td style="text-align:right;vertical-align:top;white-space:nowrap;padding-left:16px">
              ...
              <div style="font-family:monospace;font-size:11px;color:${confColor};margin-top:4px">${pick.confidence}% conf</div>
              <div style="margin-top:6px">${tierBadge(pick.riskTier)}</div>
            </td>
          </tr>
        </table>
      </td>
...
function renderTicket(ticket: Ticket, ticketNum: number): string {
  const tierColor = TIER_COLOR[ticket.riskTier] ?? "#7c3aed";
  ...
  return `
    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;background:#0f0f1a;border:1px solid #1e1e2e;border-left:3px solid ${tierColor};border-radius:6px">
      <tr>
        <td style="padding:16px 20px;border-bottom:1px solid #1e1e2e">
          <table width="100%" cellpadding="0" cellspacing="0">
            <tr>
              <td>
                <div style="font-family:monospace;font-size:11px;color:#64748b;text-transform:uppercase;letter-spacing:1px">Ticket #${ticketNum} · ${dateStr}</div>
                <div style="margin-top:6px">${tierBadge(ticket.riskTier)} <span style="font-family:monospace;font-size:11px;color:#64748b;margin-left:8px">${ticket.sport}</span></div>
              </td>
              <td style="text-align:right">
                <div style="font-family:monospace;font-size:11px;color:#64748b;text-transform:uppercase;letter-spacing:1px">Win Prob</div>
                <div style="font-family:monospace;font-size:24px;font-weight:700;color:#7c3aed">${ticket.combinedConfidence.toFixed(1)}%</div>
              </td>
            </tr>
          </table>
        </td>
...
export function buildEmailHtml(tickets: Ticket[], recipientEmail: string): string {
  ...
  const avgConf = tickets.length
    ? (tickets.reduce((sum, t) => sum + t.combinedConfidence, 0) / tickets.length).toFixed(1)
    : "0";
...
<head>
  ...
  <title>Ghostspere — Daily Pick Dispatch</title>
</head>
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%">
          ...
          <tr>
            <td style="padding-bottom:32px;text-align:center">
              ...
              <div style="font-family:monospace;font-size:11px;color:#64748b;text-transform:uppercase;letter-spacing:2px;margin-top:4px">Automated Pick Dispatch</div>
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="background:#0f0f1a;border:1px solid #1e1e2e;border-radius:6px">
                <tr>
                  <td style="padding:16px 20px;text-align:center;border-right:1px solid #1e1e2e;width:33%">
                    ...
                    <div style="font-family:monospace;font-size:10px;color:#64748b;text-transform:uppercase;letter-spacing:1px;margin-top:2px">Tickets</div>
                  </td>
                  <td style="padding:16px 20px;text-align:center;border-right:1px solid #1e1e2e;width:33%">
                  ...
                  <td style="padding:16px 20px;text-align:center;width:33%">
                    <div style="font-family:monospace;font-size:22px;font-weight:700;color:#7c3aed">${avgConf}%</div>
                    <div style="font-family:monospace;font-size:10px;color:#64748b;text-transform:uppercase;letter-spacing:1px;margin-top:2px">Avg Win Prob</div>
                  </td>
                </tr>
              </table>
            </td>
...
export function buildEmailText(tickets: Ticket[]): string {
  ...
  const lines: string[] = [
    "GHOSTSPERE — DAILY PICK DISPATCH",
    "=".repeat(40),
    dateLabel,
    "",
  ];
  ...
  for (let i = 0; i < tickets.length; i++) {
    const t = tickets[i];
    lines.push(`TICKET #${i + 1} — ${t.riskTier.toUpperCase()} PARLAY`);
    lines.push(`Win Probability: ${t.combinedConfidence.toFixed(1)}% | Sport: ${t.sport}`);
    ...
    for (const [j, pick] of t.picks.entries()) {
      ...
      lines.push(`   Confidence: ${pick.confidence}% (${pick.riskTier})`);
